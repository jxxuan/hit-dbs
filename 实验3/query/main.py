...
def parsesql(sql_statement):
    ...
    while True:
        if index >= len(sql):
            ...
            tree.lfchild.rel = sql[index - 1]
            tree.rchild = SyntaxTree()
            tree.rchild.rel = sql[index + 1]
            index += 1
        elif sql[index] == '(':
            # 遇到(再创建一个子树
            index += 1
...
def down_select(syntax_tree, sql, relation2):
    if syntax_tree.op == 'SELECT':
        ...
        for i in range(len(sql)):
            if getName(sql[i]) is not None:
                relation.append(getName(sql[i]))
                relation2.append(getRelation(sql[i]))
...
def down_proj(syntax_tree, sql, relation2, same):
    if syntax_tree.op == 'SELECT':
        ...
        for i in range(len(sql)):
            if getName(sql[i]) is not None:
                if getRelation(sql[i]) in relation2:
                    pass
                else:
                    relation2.append(getRelation(sql[i]))
        # 找相交的元素
...
def showtree(syntax_tree, g, parent=''):
    ...
    if syntax_tree.lfchild is not None:
        if syntax_tree.lfchild.rel != '':
            g.edge(this, syntax_tree.lfchild.rel)
        showtree(syntax_tree.lfchild, g, parent=this)
    if syntax_tree.rchild is not None:
        if syntax_tree.rchild.rel != '':
            g.edge(this, syntax_tree.rchild.rel)
...
def show(sql, filename):
    ...
    # 优化后的树
    opt_g = Graph(filename + 'optim', format='png', node_attr={'shape': 'plaintext'})
    # 两步优化
    relation2 = []
    opt_tree = down_select(tree, '', relation2)
    opt_tree = down_proj(opt_tree, '', relation2, '')
    showtree(opt_tree, opt_g)
    opt_g.render()
